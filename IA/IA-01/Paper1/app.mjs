...
app.post('/', async (req, res) => {
  ...
  try {
    const item = new employeeModel({ name, designation, department });
    await item.save();
...
app.put('/:id', async (req, res) => {
  try {
    const item = await employeeModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item)
      return res.status(404).json({ message: 'employee not found' });
...
app.delete('/:id', async (req, res) => {
  try {
    const item = await employeeModel.findByIdAndDelete(req.params.id);
    if (!item)
      return res.status(404).json({ message: 'employee not found' });
