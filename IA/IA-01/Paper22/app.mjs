...
app.post('/', async (req, res) => {
  const { name,rollNumber,className ,enrollmentYear} = req.body;
  try {
    const item = new StudentModel({ name, rollNumber, className ,enrollmentYear});
    await item.save();
...
app.put('/:id', async (req, res) => {
  try {
    const item = await StudentModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item)
      return res.status(404).json({ message: 'not found' });
...
app.delete('/:id', async (req, res) => {
  try {
    const item = await StudentModel.findByIdAndDelete(req.params.id);
    if (!item)
      return res.status(404).json({ message: 'not found' });
