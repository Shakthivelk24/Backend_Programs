...
router.post("/login", async (req, res) => {
  ...
  try {
    const user = await medicalModel.findOne({ userName });
...
router.post(
  "/",
  authorize(["patient"]),  // Protect this route for 'admin' role only
  async (req, res) => {
    const {patientName,doctorName,specialization,appointmentDate} = req.body;// destructuring the req body according to the schema
    try {
    const newItem = new medicalModel({ 
      patientName,                                // change the fields as per requirement given in the question
      doctorName,
      specialization,
      appointmentDate,
      notes
...
router.put("/:id",authorize(["admin"]), async (req, res) => {
  try {
    const item = await medicalModel.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
router.delete("/:id", authorize(["admin"]), async (req, res) => {
  try {
    const item = await medicalModel.findByIdAndDelete(req.params.id);
    if (!item) return res.status(404).json({ message: "Item not found" });
